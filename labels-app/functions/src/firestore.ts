...
// ユーザ管理
export const manageUser = async (accessToken: string, spotifyID: string, displayName: string, photoURL: string | null, email: string) => {
    // Spotifyのアクセストークンを保存
    ...
    await admin.auth().createCustomToken(spotifyID)
        .then(customToken => {
